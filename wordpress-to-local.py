...
commands = ['tar --exclude=\'' + domain + '/wp-content/uploads/\' -czf \'' + domain + '/' + filename + '.tar.gz\' ' + domain, 'mysqldump --add-drop-table ' + db_name + ' | gzip > ' + remote_path + filename + '.sql.gz']
...
def main():
    ...
    with gzip.open(local_path + filename + '.sql.gz', 'rb') as f_in:
        with open(local_path + filename + '.sql', 'wb') as f_out:
            shutil.copyfileobj(f_in, f_out)


    # Replace with localhost in sql dump file
    with fileinput.FileInput(local_path + filename + '.sql', inplace=True, backup='.bak') as file:
        for line in file:
